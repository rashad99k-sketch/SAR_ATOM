...
def execute_entry(side, symbol, price, sl, tp1, tp2, score, reason, atr_val, trade_type, entry_type, classification):
    """Enhanced entry with ADX and liquidity checks from test cv new.py."""
    # ---- ADX and Liquidity Sweep checks ----
    df = get_ohlcv_safe(symbol, 100)
    if df is not None:
        adx_series = compute_adx(df)
        adx_val = adx_series.iloc[-1] if adx_series is not None and len(adx_series) > 0 else 20.0
        # ADX range 25-38 (as per test cv new.py)
        ...
        if side == "SELL" and liquidity_ctx != "buy_side_taken":
            ...
            return False
        # Sweep authenticity: the gate must not chase a FAKE sweep (body
        # filled through the level with no counter-wick and no reclaim).
        if SWEEP_AUTHENTICITY:
            sweep_grad, sweep_bar = get_sweep_authenticity(df, side, lookback=10)
            if sweep_grad == "fake":
                log_execution(
                    f"[ENTRY] {side} sweep on bar {sweep_bar} is FAKE "
                    f"(no counter-wick/reclaim) – refusing chase", "WARN")
                return False
    else:
        log_execution(f"[ENTRY] No DF for ADX/liquidity check, skipping", "WARN")
        return False
...
def promote_to_queue():
    ...
    for sym, entry in watchlist.items():
        ...
        if not analysis:
            continue
        if now - analysis.get("last_analyzed", 0) > 300:
            _watchlist_analysis_stats["stale_analysis"] += 1
            continue
        if analysis.get("ob_grade", "NONE") == "INVALID":
            continue
        if analysis.get("trap_risk", 50) > 70:
            continue
    ...
    candidates.sort(key=lambda x: x["score"], reverse=True)
    to_promote = candidates[:min(10, len(candidates))]
    promoted = 0
    for item in to_promote:
        ...
        cand = ExecutionCandidate(
            ...
            df=None,
            ob=None,
            original_score=analysis.get("composite_score", 0),
            original_reason="Watchlist promotion (pre-analyzed)",
            signal_type="watchlist_staggered",
            ob_cfg=AssetBehaviorProfile.ob_config(AssetBehaviorProfile.resolve_asset_class(sym))
        )
        cand.roro_signal = analysis.get("roro_signal", False)
        cand.strong_ob_present = analysis.get("strong_ob", False)
        cand.ob_grade = analysis.get("ob_grade", "NONE")
        cand.is_a_grade = cand.strong_ob_present and cand.roro_signal
        cand.priority_score = analysis.get("composite_score", 0)
        ...
        if queue.add_candidate(cand):
            promoted += 1
            _watchlist_analysis_stats["promoted"] += 1
            log_execution(f"[PROMOTE] {sym} {side} promoted to Queue (score={cand.priority_score:.1f})", "INFO")
    if promoted:
        _watchlist_analysis_stats["top_candidates"] = [{"symbol": c["symbol"], "score": c["score"]} for c in to_promote[:5]]
...
class InstitutionalRadar:
    ...
    def _promote_to_queue(self, symbol, entry):
        ...
        log_execution(
            f"[RADAR] {symbol} promoted to Queue (score={cand.institutional_score:.1f}, accel={cand.institutional_acceleration:.1f})",
            "SUCCESS"
        )
